...
void HttpServer::dataReceiveCallBack(Socket* socket, ByteArray* buffer, void* data, int count)
{
	if (count > 0)
	{
		log_WriteLine<HttpServer>(log_dbg, "From " + socket->ipEndPoint()->toString());
		log_WriteLine<HttpServer>(log_dbg, "Received data length = " + std::to_string(count));
		...
		log_WriteLine<HttpServer>(log_dbg, "Response : " + resp.statusCodeString());
...
HttpResponse HttpServer::processIncomingRequest(const HttpRequest& request)
{
	...
	log_WriteLine<HttpServer>(log_dbg, "Requesting path = " + request.path());
	...
	if (realPath == "")
	{
		log_WriteLine<HttpServer>(log_dbg, "No Access to this directory !");
	...
	if (stream.fail())
	{
		log_WriteLine<HttpServer>(log_err, "Cannot open file : " + realPath);
		resp.setStatusCode(404);
		resp.setData(http_response_404);
	}
	else
	{
		log_WriteLine<HttpServer>(log_inf, "Reading file : " + realPath);
		ByteArray data;
		while (!stream.eof())
		{
			char buf[1024];
